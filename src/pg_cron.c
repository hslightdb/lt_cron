...
static bool CronLogStatement = false;
...
_PG_init(void)
{
	...
	if (!process_shared_preload_libraries_in_progress)
	{
		//ereport(ERROR, (errmsg("pg_cron can only be loaded via shared_preload_libraries"),
						//errhint("Add pg_cron to the shared_preload_libraries "
								//"configuration variable in lightdb.conf.")));
		return;
	}

	DefineCustomStringVariable(
		"cron.database_name",
		gettext_noop("Database in which lt_cron metadata is kept."),
		NULL,
	...
	DefineCustomStringVariable(
		"cron.host",
		gettext_noop("Hostname to connect to LightDB."),
	...
	snprintf(worker.bgw_name, BGW_MAXLEN, "lt_cron launcher");
#if (PG_VERSION_NUM >= 110000)
	snprintf(worker.bgw_type, BGW_MAXLEN, "lt_cron launcher");
#endif
...
PgCronLauncherMain(Datum arg)
{
	...
	pgstat_report_appname("lt_cron scheduler");
	...
	ereport(LOG, (errmsg("lt_cron scheduler started")));
	...
	ereport(LOG, (errmsg("lt_cron scheduler shutting down")));

	proc_exit(0);
}

/* 
 * delete earliest data from cron.job_run_details where raw > 100000 every 3 minutes
...
clearJobRunDetails(void)
{
	...
	if (CLEAR_RUN_DETAILS_INTERVAL <= (currentTime - g_lastKeepRunDetailes))
	{
		keepDataFromCronRun();
		g_lastKeepRunDetailes = currentTime;
	}
...
StartPendingRuns(CronTask *task, TimestampTz currentTime)
{
	...
	if (is_second)
	{
		...
		*/
		...
		if (secondsPassed == 0)
		{
			/* wait for new second, nerver occur. 
			 * TODO currently, if there is at least one task, lt_cron will run infinite without sleep, this will result lt_cron process's cpu too busy. 
			 * so sleep some time, for example 100ms, will be a more better choice. the more reasonable interval can be calculate as follows:
			 * 1000ms - the elapse that scan all task * 3.
			 */
			//ereport(LOG, (errmsg("lt_cron launcher wait at least 1s, so shouldn't occur, #%ld,  %d.", task->jobId, task->state)));
			return;
		}
		...
		{
			/* clock jumped forward by more than 3 minutes */
...
ManageCronTask(CronTask *task, TimestampTz currentTime)
{
	...
	switch (checkState)
	{
		case CRON_TASK_WAITING:
		{
			...
			if (task->iscancel && checkPidFromActivity(task->pid))
				break;
			else
			{
				task->pid = 0;
...
#if (PG_VERSION_NUM >= 110000)
			snprintf(worker.bgw_type, BGW_MAXLEN, "lt_cron");
#endif
			snprintf(worker.bgw_name, BGW_MAXLEN, "lt_cron worker");
...
CronBackgroundWorker(Datum main_arg)
{
	...
	Assert(CurrentResourceOwner == NULL);
	CurrentResourceOwner = ResourceOwnerCreate(NULL, "lt_cron");
		...
		if (IsA(parsetree, TransactionStmt))
			ereport(ERROR,
					(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
					 errmsg("transaction control statements are not allowed in lt_cron")));
