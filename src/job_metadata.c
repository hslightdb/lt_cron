...
CronExtensionOwner(void)
{
	...
	if (!HeapTupleIsValid(extensionTuple))
	{
		ereport(ERROR, (errcode(ERRCODE_OBJECT_NOT_IN_PREREQUISITE_STATE),
						errmsg("lt_cron extension not loaded")));
	}
...
TupleToCronJob(TupleDesc tupleDescriptor, HeapTuple heapTuple)
{
	...
	{
		ereport(LOG, (errmsg("invalid lt_cron schedule for job " INT64_FORMAT ": %s",
...
insertCronExt(int64 jobid, char *jobname, char *mode, char *tmzone, char *cmdtype)
{
	...
	if(!JobLtExtTableExists() || RecoveryInProgress())
	{
		return;
	}
...
keepDataFromCronRun()
{
	...
	appendStringInfo(&deletebuf, "delete from %s where runid < (currval('cron.runid_seq') - %d)",
			quote_qualified_identifier(CRON_SCHEMA_NAME, JOB_RUN_DETAILS_TABLE_NAME), MAX_RUN_DETAILS_ROW_NUM);
 ...
 */
bool
checkPidFromActivity(pid_t pid)
{
	StringInfoData querybuf;
	MemoryContext originalContext = CurrentMemoryContext;
	char *buf = NULL;
	...
	appendStringInfo(&querybuf, "select pid from pg_stat_activity where application_name = 'lt_cron'");
	...
	if (SPI_processed > 0)
	{
		for (unsigned int i = 0; i < SPI_tuptable->numvals; i++)
		{
			buf = SPI_getvalue(SPI_tuptable->vals[i], SPI_tuptable->tupdesc, 1);
			if (NULL != buf && atoi(buf) == pid)
			{
				ret = true;
				break;
			}
